"""
High-intrinsic-Q microring DOE for the SiEPIC openEBL-2026-05 run.
...
DESIGNER = "BiCinur"
TECH_NAME = "EBeam"
TOP_CELL_NAME = "openEBL_BiCinur_highQ"
EXPORT_TYPE = "static"
...
HIGHQ_DEVICE_ROTATION = Trans.R270
HIGHQ_DEVICE_OFFSET_X_UM = 110.0
CELL_WIDTH_UM = 605.0
...
DEVICES = [
    DeviceSpec("HQ01", "custom_ring", "HQ_R80_W1500_B1028_G125_T20", 80, 1.5, 1.028, 0.125, 20),
    DeviceSpec("HQ02", "custom_ring", "HQ_R80_W1500_B1028_G150_T20", 80, 1.5, 1.028, 0.150, 20),
    DeviceSpec("HQ03", "custom_ring", "HQ_R80_W1500_B1028_G175_T20", 80, 1.5, 1.028, 0.175, 20),
    DeviceSpec("HQ04", "custom_ring", "HQ_R80_W2000_B1028_G125_T20", 80, 2.0, 1.028, 0.125, 20),
    DeviceSpec("HQ05", "custom_ring", "HQ_R80_W2000_B1028_G150_T20_REP1", 80, 2.0, 1.028, 0.150, 20, "limit"),
    DeviceSpec("HQ06", "custom_ring", "HQ_R80_W2000_B1028_G150_T20_REP2", 80, 2.0, 1.028, 0.150, 20, "limit"),
    DeviceSpec("HQ07", "straight_reference", "HQ_REF_STRAIGHT500"),
    DeviceSpec("HQ08", "taper_reference", "HQ_REF_TAPER500_1028", bus_width_um=1.028),
    DeviceSpec("HQ09", "pdk_control_ring", "HQ_CTRL_PDK_R40_W500_G200", 40, 0.5, 0.5, 0.200),
]

# The fixed openEBL floorplan only supports two independent 127-um GC rows.
# Once routed beside the grating-coupler pair, each R80 circuit consumes enough
# width that a clean single-file v1 can host four independent high-Q devices,
# not the original six-ring sweep. We keep the most information-dense 2x2
# matrix: two widths crossed with two gaps. Branch-local references remain
# documented in the registry but are deferred in v1; the photon-source file
# carries the shared straight/taper/control rails.
PLACED_DEVICE_IDS = {"HQ01", "HQ02", "HQ04", "HQ05"}
...
def place_two_port_device(
    ...
    inst_device = cell.insert(
        CellInstArray(
            device_cell.cell_index(),
            Trans(rotation, um(x_um + device_offset_x_um, dbu), um(y_um + GC_PITCH_UM / 2.0, dbu)),
        )
    )
...
def build_layout() -> tuple[pya.Layout, pya.Cell]:
    ...
    placed_devices = [spec for spec in DEVICES if spec.device_id in PLACED_DEVICE_IDS]
    for spec, (x_um, y_um) in zip(placed_devices, positions):
        ...
        if spec.kind == "custom_ring":
            ...
            _, _, inst_device = place_two_port_device(
                top_cell,
                layout,
                gc_cell,
                device_cell,
                ...
                y_um,
                label,
                HIGHQ_DEVICE_OFFSET_X_UM,
                Trans.R90,
            )
            if spec.device_id == "HQ05":
                anchor_instance = inst_device
                sidewall_sem_center = (
                    x_um + HIGHQ_DEVICE_OFFSET_X_UM - spec.radius_um,
                    y_um + GC_PITCH_UM / 2.0,
                )
        else:
